...
def render_summary(result: AnalysisResult) -> None:
    ...
    st.subheader("Sintesi", divider="gray")

    c1, c2, c3 = st.columns(3)
    c1.metric("Variazione totale", f"{s['total_variation']:.2f} M",
              help="Somma dei valori assoluti delle altezze differenziali. "
                   "Non lineare, quindi distingue percorsi diversi anche a "
                   "parita' di estremi.")
    c2.metric("Salto massimo", f"{s['max_jump']:+.2f} M",
              help="Massima intensificazione fra due bin consecutivi.")
    c3.metric("Concentrazione", f"{result.concentration:.1%}",
              help="Frazione dell'energia totale dovuta ai soli eventi massimi "
                   "di ciascun bin.")

    with st.expander("Differenza fra gli estremi (lineare)"):
        st.caption(
            "La somma con segno delle altezze telescopizza: vale la differenza "
            "fra l'ultimo e il primo bin, indipendentemente da tutto quello che "
            "accade nel mezzo. E' una quantita' legittima ma va letta per quello "
            "che e', non come un bilancio del periodo."
        )
        st.metric("Δ magnitudo equivalente, primo → ultimo bin", f"{s['net_m']:+.2f} M")
...
def render_differentials(result: AnalysisResult) -> None:
    """Altezze differenziali e derivata locale."""
    d = result.diff
    if d.empty:
        return

    st.subheader("Analisi differenziale", divider="gray")
    ...
    fig.add_trace(go.Bar(
        ...
        name="Δ magnitudo equivalente",
    ))
    _target_marker(fig, result.config.target_year)
    fig.update_layout(template="plotly_white", height=340, showlegend=False,
                      xaxis_title="tempo",
                      yaxis_title="Δ magnitudo equivalente",
                      margin=dict(l=0, r=0, t=30, b=0))
    st.plotly_chart(fig, use_container_width=True)
    st.caption(
        "Verde: il bin ha rilasciato piu' energia del precedente. "
        "Rosso: ne ha rilasciata meno. Le barre sbiadite coinvolgono un bin "
        "vuoto, dove l'altezza misura la distanza dalla soglia di completezza "
        "e non una variazione fra due stati osservati."
    )
    ...
    if d["slope_m_yr"].notna().any():
        ...
        fig3.add_trace(go.Scatter(
            ...
            line=dict(color=ACC, width=2), name="pendenza",
        ))
        ...
        st.caption(
            "Regressione locale ai minimi quadrati, piu' stabile della "
            "differenza a un passo rispetto al rumore di singolo evento. "
            "I bin vuoti sono esclusi dal fit."
        )
...
def render_annual_comparison(result: AnalysisResult) -> None:
    ...
    Sostituisce l'overlay delle aree cumulate. Quella costruzione sommava le
    aree mese per mese, ma la somma telescopizza: la curva risultante era la
    serie energetica stessa riscalata di un fattore costante, e l'asse
    verticale non aveva l'unita' di misura che dichiarava. La domanda
    sottostante resta valida e viene qui posta sulla grandezza osservata.
    """
    matrix = annual_matrix(result.binned, "m_eq")
    if matrix.empty or len(matrix) < 2:
        return

    st.subheader("Confronto annuale", divider="gray")
    ...
    fig.add_trace(go.Scatter(
        ...
        name="inviluppo 10–90% degli anni di riferimento", hoverinfo="skip",
    ))
    fig.add_trace(go.Scatter(
        x=x, y=med, mode="lines+markers", name="mediana di riferimento",
    ...
    if target in matrix.index:
        fig.add_trace(go.Scatter(
            ...
            name=f"anno di verifica {target}",
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption(
        f"Ogni mese dell'anno di verifica e' confrontato con la distribuzione "
        f"dello stesso mese negli anni di riferimento ({reference.index.min()}–"
        f"{reference.index.max()}). Un punto fuori dall'inviluppo indica un mese "
        "atipico rispetto alla storia locale, non una previsione."
    )

    with st.expander("Matrice anno × mese"):
